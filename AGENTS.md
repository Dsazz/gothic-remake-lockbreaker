...
- **Keyboard:** `shortcuts_opened` (`source` = icon/key/new_badge; mirrors `guide_opened`; `new_badge` = opened via the icon while the "NEW" feature badge was showing), `keyboard_nav_used` (`surface` = walkthrough/mapping; fires at most once per session on first keyboard navigation).
